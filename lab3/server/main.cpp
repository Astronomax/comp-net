...
int main(int argc, char *argv[]) {
    ...
    listen(sock_fd, 5);
    std::unordered_set<pthread_t> session_threads;
    printf("Waiting for clients...\n");
    while(true) {
        sockaddr_in cli_addr{};
        socklen_t cli_len = sizeof(cli_addr);
        int new_sock_fd = accept(sock_fd, reinterpret_cast<sockaddr *>(&cli_addr), &cli_len);
        if (new_sock_fd < 0) {
        ...
        pthread_create(&new_thread, nullptr, session_routine, new session_routine_args {new_sock_fd});
        session_threads.insert(new_thread);
    }
}
